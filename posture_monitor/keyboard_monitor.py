...
from posture_monitor.keyboard_session_config import posture_session_args
            ...
            with mp_pose.Pose(
                ...
                while camera_on and cap.isOpened():
                    ...
                    if results.pose_landmarks:
                        ...
                        print(f"y_diff {np.round(np.abs(landmark_lst[16].y-landmark_lst[15].y), 2)}")
                        #print(np.round((landmark_lst[16].x, landmark_lst[16].y),2))
                        print(f"center_coordinate {center}")
                        #print(f"width {image_flip.shape[1]}, height {image_flip.shape[0]}")
                        image = cv2.circle(image, center=center, radius=30, color=(255, 0, 0), thickness=5)
                        #image_flip = cv2.circle(image_flip, (landmark_lst[20].x, landmark_lst[20].y), 2,  color=(0, 255, 0), thickness=2)
